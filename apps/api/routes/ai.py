...
async def generate_insight(
    patient_id: str, 
    persona: str = Query("patient", description="Persona for the generated insight, e.g., 'patient' or 'clinician'"),
    ...
    try:
        ...
        time_in_range = await AnalyticsService.get_time_in_range(db, patient_id, start_date, end_date)
        daily_summary = await AnalyticsService.get_daily_summary(db, patient_id, end_date.date())
        weekly_trends = await AnalyticsService.get_weekly_trends(db, patient_id)
        ...
        insight = ai_engine.generate_insight(persona, data_context)
        return insight
